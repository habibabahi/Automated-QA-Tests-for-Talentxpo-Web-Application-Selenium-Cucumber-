...
@CucumberOptions(
        ...
        plugin = {"pretty", "json:target/json/results.json"}, // Génère un rapport de test lisible (`pretty`) et un fichier JSON dans le dossier `target/json`.

        // Option `features` pour indiquer le chemin vers le dossier contenant les fichiers de scénarios `.feature`.
        features = {"src/test/resources/Features"}, // Chemin relatif vers le dossier où se trouvent les fichiers de caractéristiques.
